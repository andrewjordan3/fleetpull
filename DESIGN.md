...
- *The legacy `GET /v1/fleet/trips` is the only surface* (the modern candidates 404) and
  ...
  change. The per-trip wire record does NOT echo `vehicleId` (the fan-out member), so the
  binding's `SamsaraTripsPageDecoder` stamps it off the sent spec onto every record — the
  sole synthesized field, a string joining `Vehicle.id`, so a stored trip is attributable to
  its vehicle (the window-report sent-spec stamp applied to a fan-out member). Retrieval is
  overlap-anchored (shared-principle case; a 60-second window strictly inside a trip returned
  it).
...
| Samsara | The `/v1/fleet/trips` envelope is `{"trips": [...]}` — no pagination of any kind; one response per (vehicle, window), so a terminal single-page decode fits. But the wire record carries no vehicle id, so trips binds `SamsaraTripsPageDecoder`, which stamps the fan-out `vehicleId` off the sent spec onto every record (the `SamsaraWindowReportPageDecoder` sent-spec stamp applied to a fan-out member; envelope captured 2026-07-20, stamp added 2026-07-22). |
...
| Samsara | Trip-record census (725 trips, 60 vehicles, ZERO nulls anywhere): always present — `startMs`/`endMs` (epoch-ms ints; `endMs` on every observed trip including the two most recent — in-progress trips were never observed and appear to materialize on completion; the lookback absorbs late materialization, an accepted residual), `distanceMeters`, `fuelConsumedMl`, `tollMeters`, `startOdometer`/`endOdometer` (ints, provider units mirrored verbatim), `driverId` (int; 0 is the UNASSIGNED sentinel, 110/725), `startLocation`/`endLocation` (reverse-geocoded strings), `startCoordinates`/`endCoordinates` (`{latitude, longitude}` floats), `assetIds`/`codriverIds` (`list[int]`; the census saw only empties, but a full-scale live pull 2026-07-22 returned `assetIds` populated on a minority — attached assets, not the trip's own vehicle — the `list[int]` typing absorbing it; `codriverIds` stayed empty). Partial: `startAddress` 177/725 and `endAddress` 185/725, each `{address: str, id: int, name: str}` — present when the trip endpoint matched a defined address/geofence. The wire record does NOT echo the requested `vehicleId`, so `SamsaraTripsPageDecoder` stamps it off the sent spec (fix added 2026-07-22) — the sole synthesized field, a string joining `Vehicle.id` (captured 2026-07-20). |

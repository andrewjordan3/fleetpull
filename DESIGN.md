...
7. **GeoTab verticals.** GeoTab is the architectural stress test (different
   ...
   `event_time_column='start'`. The abstractions held — no driver, runner, or
   entry change carried a provider branch. The feed vertical remains deferred.
...
the program), a separate CLI wrapper, feed-mode execution/storage, and schema
overrides for unsupported model shapes.

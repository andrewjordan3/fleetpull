...
class GeotabFeedPageDecoder:
    ...
    def decode_page(self, sent: RequestSpec, envelope: JsonValue) -> DecodedPage:
        ...
        sent_from_version = sent_params.get(_FROM_VERSION_KEY)
        if sent_from_version is not None and not isinstance(sent_from_version, str):
            raise ValueError(
                f'GeoTab feed request params must carry a string {_FROM_VERSION_KEY!r}'
            )
        ...
        if sent_from_version == feed.to_version:
            raise ProviderResponseError(
                provider=None,
                endpoint=None,
                detail=(
                    'GeoTab GetFeed returned a full continuing page without '
                    'advancing toVersion'
                ),
            )

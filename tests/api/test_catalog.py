...
from fleetpull.api import IncrementalEndpoint, SnapshotEndpoint, available_endpoints
...
def _identity_type_for_mode(
    sync_mode: SyncMode,
) -> type[SnapshotEndpoint] | type[IncrementalEndpoint]:
...
def test_wrong_typed_catalog_identity_is_reported() -> None:
    wrong_typed = IncrementalEndpoint(Provider.MOTIVE, 'vehicles')
    modes: dict[_RegistryKey, SyncMode] = {
        (Provider.MOTIVE, 'vehicles'): SnapshotMode()
    }
    violations = _parity_violations([wrong_typed], modes)
    assert len(violations) == 1
    assert 'IncrementalEndpoint' in violations[0]
    assert 'SnapshotEndpoint' in violations[0]


def test_feed_mode_maps_to_incremental_endpoint() -> None:
    assert _identity_type_for_mode(FeedMode()) is IncrementalEndpoint

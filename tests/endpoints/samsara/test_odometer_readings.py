"""Tests for fleetpull.endpoints.samsara.odometer_readings.

The odometer arm of the vehicle-stats triple (probe-settled
2026-07-20): a fleet-wide ``SingleFetch`` whose shared leaf builder
renders the resume window as RFC3339 ``startTime``/``endTime`` plus the
FIXED ``types=obdOdometerMeters`` selector, paired with
``SamsaraVehicleSeriesPageDecoder`` at the surface's probed 512-limit
tier. This module pins the binding's own facts; the class-level
builder behaviors shared by the three leaves are pinned once in
``test_engine_states``.
"""
...
from fleetpull.endpoints.samsara.odometer_readings import build_endpoint
...
def _build_endpoint() -> EndpointDefinition[OdometerReading]:
...
class TestOdometerReadingsSpecBuilder:
...
class TestBuildOdometerReadingsEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.SAMSARA
        assert endpoint.name == 'odometer_readings'
        assert endpoint.response_model is OdometerReading
    ...
    def test_uses_the_series_decoder_at_the_512_limit_tier(self) -> None:
        ...
        assert decoder.series_key == 'obdOdometerMeters'
    ...
    def test_the_window_and_type_ride_every_page_of_the_walk(self) -> None:
        ...
        continued = endpoint.page_decoder.decode_page(
            first, ODOMETER_READINGS_PAGE_RESPONSE
        )
        ...
        assert next_spec.params == {
            ...
            'endTime': '2026-01-08T00:00:00Z',
            'limit': '512',
            'after': '00000000-0000-0000-0000-000000000061',
        }
        terminal = endpoint.page_decoder.decode_page(
            next_spec, ODOMETER_READINGS_TERMINAL_RESPONSE
        )
    ...
    def test_constructs_without_raising(self) -> None:
        ...
        # validation against the OdometerReading model.

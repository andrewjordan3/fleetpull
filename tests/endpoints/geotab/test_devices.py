"""Tests for fleetpull.endpoints.geotab.devices.

The sort assertion is the load-bearing positive here: the id-sort seek
walk was probed on ``Device`` first (the users leaf later re-proved it
for ``User`` rather than assuming it), and the first-page shape
(``sortBy: id``, ascending, an EXPLICIT null ``offset``) is the probed
...
from fleetpull.endpoints.geotab.devices import build_endpoint
...
def _build_endpoint() -> EndpointDefinition[Device]:
    return build_endpoint(GeotabConfig())


class TestDevicesSpecBuilder:
...
class TestBuildDevicesEndpoint:
    ...
    def test_the_completeness_check_counts_devices(self) -> None:
        endpoint = _build_endpoint()
        check = endpoint.completeness_check
        assert isinstance(check, GetCountOfCheck)
        assert check.type_name == 'Device'

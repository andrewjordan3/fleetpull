"""Tests for fleetpull.endpoints.geotab.driver_changes.
...
from fleetpull.endpoints.geotab.driver_changes import build_endpoint
...
from tests.geotab_driver_changes_capture import (
    DRIVER_CHANGES_FEED_PAGE_1_RESPONSE,
    DRIVER_CHANGES_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[DriverChange]:
    return build_endpoint(GeotabConfig())


class TestBuildDriverChangesEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'driver_changes'
        ...
        assert endpoint.response_model is DriverChange
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'DriverChange'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'DriverChange'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        result = drive_feed_endpoint(
            _build_endpoint(),
            [DRIVER_CHANGES_FEED_PAGE_1_RESPONSE, DRIVER_CHANGES_FEED_PAGE_2_RESPONSE],
            tmp_path,
            page_size=2,
        )
        ...
        assert result.cursor == FeedToken(from_version='00000000000023b3')
        ...
        assert second_params['fromVersion'] == '00000000000023b2'

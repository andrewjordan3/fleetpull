"""Tests for fleetpull.endpoints.geotab.fault_data.
...
cursor store. The shared ``GeotabGetFeedSpecBuilder`` wire shapes are
...
from fleetpull.endpoints.geotab.fault_data import build_endpoint
...
from tests.geotab_fault_data_capture import (
    FAULT_DATA_FEED_PAGE_1_RESPONSE,
    FAULT_DATA_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[FaultData]:
    return build_endpoint(GeotabConfig())


class TestBuildFaultDataEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'fault_data'
        ...
        assert endpoint.response_model is FaultData
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'FaultData'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'FaultData'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        result = drive_feed_endpoint(
            _build_endpoint(),
            [FAULT_DATA_FEED_PAGE_1_RESPONSE, FAULT_DATA_FEED_PAGE_2_RESPONSE],
            tmp_path,
            page_size=2,
        )
        ...
        # The LogRecord asymmetry, mirrored into storage: NO version
        # column on this active feed.
        assert 'version' not in combined.columns
        assert sorted(combined['id'].to_list()) == [
            'b21f201',
            'b21f202',
            'b21f203',
        ]
        assert result.cursor == FeedToken(from_version='00000000000021f3')
        ...
        assert second_params['fromVersion'] == '00000000000021f2'

"""Tests for fleetpull.endpoints.geotab.fuel_tax_details.
...
cursor store. The load-bearing leaf pin here is
``event_time_column='enter_time'`` — the segment materializes where it
begins. The shared ``GeotabGetFeedSpecBuilder`` wire shapes are pinned
in ``test_requests.py``.
"""
...
from fleetpull.endpoints.geotab.fuel_tax_details import build_endpoint
...
from tests.geotab_fuel_tax_details_capture import (
    FUEL_TAX_DETAILS_FEED_PAGE_1_RESPONSE,
    FUEL_TAX_DETAILS_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[FuelTaxDetail]:
    return build_endpoint(GeotabConfig())


class TestBuildFuelTaxDetailsEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'fuel_tax_details'
        ...
        assert endpoint.response_model is FuelTaxDetail
        assert endpoint.event_time_column == 'enter_time'
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'FuelTaxDetail'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'FuelTaxDetail'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        ...
        # Rows route by enter_time — the declared event-time column.
        ...
        # The list columns (empty arrays included) and the versions list
        # survive the whole drive into storage.
        assert combined.schema['versions'] == pl.List(pl.String)
        empty_row = combined.filter(pl.col('id') == 'b18e402')
        assert empty_row['hourly_odometer'].to_list() == [[]]
        assert result.cursor == FeedToken(from_version='00000000000018e4')
        ...
        assert second_params['fromVersion'] == '00000000000018e3'

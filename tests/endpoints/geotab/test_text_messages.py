"""Tests for fleetpull.endpoints.geotab.text_messages.
...
cursor store. This vertical's event_time_column is ``sent`` (NO
``dateTime``) and it carries NO per-record ``version`` (the append-only
asymmetry) — both pinned here. The shared ``GeotabGetFeedSpecBuilder``
wire shapes are pinned in ``test_requests.py``; here only the leaf's
composed values are.
"""
...
from fleetpull.endpoints.geotab.text_messages import build_endpoint
...
from tests.geotab_text_messages_capture import (
    TEXT_MESSAGES_FEED_PAGE_1_RESPONSE,
    TEXT_MESSAGES_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[TextMessage]:
    return build_endpoint(GeotabConfig())


class TestBuildTextMessagesEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'text_messages'
        ...
        assert endpoint.response_model is TextMessage
        # The event-time departure: sent, not date_time (no dateTime key).
        assert endpoint.event_time_column == 'sent'
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'TextMessage'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'TextMessage'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        result = drive_feed_endpoint(
            _build_endpoint(),
            [TEXT_MESSAGES_FEED_PAGE_1_RESPONSE, TEXT_MESSAGES_FEED_PAGE_2_RESPONSE],
            tmp_path,
            page_size=2,
        )
        ...
        # Partitioned by sent (the event time), not dateTime.
        ...
        # The append-only asymmetry, mirrored into storage: NO version
        # column on this feed.
        assert 'version' not in combined.columns
        assert sorted(combined['id'].to_list()) == ['bTM201', 'bTM202', 'bTM203']
        # The feed's own toVersion still advances the cursor.
        assert result.cursor == FeedToken(from_version='0000000000002d03')
        ...
        assert second_params['fromVersion'] == '0000000000002d02'

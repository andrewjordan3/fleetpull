"""Tests for fleetpull.endpoints.geotab.annotation_logs.
...
from fleetpull.endpoints.geotab.annotation_logs import build_endpoint
...
from tests.geotab_annotation_logs_capture import (
    ANNOTATION_LOGS_FEED_PAGE_1_RESPONSE,
    ANNOTATION_LOGS_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[AnnotationLog]:
    return build_endpoint(GeotabConfig())


class TestBuildAnnotationLogsEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'annotation_logs'
        ...
        assert endpoint.response_model is AnnotationLog
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'AnnotationLog'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'AnnotationLog'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        ...
        assert result.cursor == FeedToken(from_version='0000000000002a03')
        ...
        assert second_params['fromVersion'] == '0000000000002a02'

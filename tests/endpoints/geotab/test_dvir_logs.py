"""Tests for fleetpull.endpoints.geotab.dvir_logs.
...
cursor store. The wire ``typeName`` keeps the provider's own
``'DVIRLog'`` casing where the model is the house-cased ``DvirLog`` —
pinned here. The shared ``GeotabGetFeedSpecBuilder`` wire shapes are
pinned in ``test_requests.py``; here only the leaf's composed values
are.
"""
...
from fleetpull.endpoints.geotab.dvir_logs import build_endpoint
...
from tests.geotab_dvir_logs_capture import (
    DVIR_LOGS_FEED_PAGE_1_RESPONSE,
    DVIR_LOGS_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[DvirLog]:
    return build_endpoint(GeotabConfig())


class TestBuildDvirLogsEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'dvir_logs'
        ...
        assert endpoint.response_model is DvirLog
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        # The wire's own DVIR casing, not the house-cased model name.
        endpoint = _build_endpoint()
        assert isinstance(endpoint.spec_builder, GeotabGetFeedSpecBuilder)
        assert endpoint.spec_builder.type_name == 'DVIRLog'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'DVIRLog'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        result = drive_feed_endpoint(
            _build_endpoint(),
            [DVIR_LOGS_FEED_PAGE_1_RESPONSE, DVIR_LOGS_FEED_PAGE_2_RESPONSE],
            tmp_path,
            page_size=2,
        )
        ...
        # certified/edited-log (id, max version) reconcile key.
        assert sorted(combined['version'].to_list()) == [
            '00000000000024c1',
            '00000000000024c2',
            '00000000000024c3',
        ]
        assert result.cursor == FeedToken(from_version='00000000000024c3')
        ...
        assert second_params['fromVersion'] == '00000000000024c2'

"""Tests for fleetpull.endpoints.geotab.status_data.
...
from fleetpull.endpoints.geotab.status_data import build_endpoint
...
from tests.geotab_status_data_capture import (
    STATUS_DATA_FEED_PAGE_1_RESPONSE,
    STATUS_DATA_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[StatusData]:
    return build_endpoint(GeotabConfig())


class TestBuildStatusDataEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'status_data'
        ...
        assert endpoint.response_model is StatusData
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'StatusData'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        result = drive_feed_endpoint(
            _build_endpoint(),
            [STATUS_DATA_FEED_PAGE_1_RESPONSE, STATUS_DATA_FEED_PAGE_2_RESPONSE],
            tmp_path,
            page_size=2,
        )
        ...
        # The per-record versions ride into storage — the mirrored
        # StatusData asymmetry.
        assert sorted(combined['version'].to_list()) == [
            '00000000000015b1',
            '00000000000015b2',
            '00000000000015b3',
        ]
        assert result.cursor == FeedToken(from_version='00000000000015b3')
        ...
        assert second_params['fromVersion'] == '00000000000015b2'

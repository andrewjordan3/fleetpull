"""Tests for fleetpull.endpoints.geotab.shipment_logs.
...
from fleetpull.endpoints.geotab.shipment_logs import build_endpoint
...
from tests.geotab_shipment_logs_capture import (
    SHIPMENT_LOGS_FEED_PAGE_1_RESPONSE,
    SHIPMENT_LOGS_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[ShipmentLog]:
    return build_endpoint(GeotabConfig())


class TestBuildShipmentLogsEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'shipment_logs'
        ...
        assert endpoint.response_model is ShipmentLog
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'ShipmentLog'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'ShipmentLog'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        result = drive_feed_endpoint(
            _build_endpoint(),
            [SHIPMENT_LOGS_FEED_PAGE_1_RESPONSE, SHIPMENT_LOGS_FEED_PAGE_2_RESPONSE],
            tmp_path,
            page_size=2,
        )
        ...
        assert result.cursor == FeedToken(from_version='0000000000002b03')
        ...
        assert second_params['fromVersion'] == '0000000000002b02'

"""Tests for fleetpull.endpoints.geotab.duty_status_logs.
...
from fleetpull.endpoints.geotab.duty_status_logs import build_endpoint
...
from tests.geotab_duty_status_logs_capture import (
    DUTY_STATUS_LOGS_FEED_PAGE_1_RESPONSE,
    DUTY_STATUS_LOGS_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[DutyStatusLog]:
    return build_endpoint(GeotabConfig())


class TestBuildDutyStatusLogsEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'duty_status_logs'
        ...
        assert endpoint.response_model is DutyStatusLog
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'DutyStatusLog'
    ...
    def test_credentials_are_never_written_here(self) -> None:
        ...
        assert params['typeName'] == 'DutyStatusLog'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        ...
        # The per-record versions ride into storage — the editable-log
        ...
        assert result.cursor == FeedToken(from_version='00000000000022a3')
        ...
        assert second_params['fromVersion'] == '00000000000022a2'

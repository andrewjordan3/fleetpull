"""Tests for fleetpull.endpoints.geotab.audits.
...
from fleetpull.endpoints.geotab.audits import build_endpoint
...
from tests.geotab_audits_capture import (
    AUDITS_FEED_PAGE_1_RESPONSE,
    AUDITS_FEED_PAGE_2_RESPONSE,
)


def _build_endpoint() -> EndpointDefinition[Audit]:
    return build_endpoint(GeotabConfig())


class TestBuildAuditsEndpoint:
    def test_binds_the_static_facts(self) -> None:
        endpoint = _build_endpoint()
        assert endpoint.provider is Provider.GEOTAB
        assert endpoint.name == 'audits'
        ...
        assert endpoint.response_model is Audit
    ...
    def test_composes_the_shared_feed_builder(self) -> None:
        ...
        assert endpoint.spec_builder.type_name == 'Audit'
...
class TestFeedDrive:
    def test_pages_land_as_parts_and_the_token_advances(self, tmp_path: Path) -> None:
        result = drive_feed_endpoint(
            _build_endpoint(),
            [AUDITS_FEED_PAGE_1_RESPONSE, AUDITS_FEED_PAGE_2_RESPONSE],
            tmp_path,
            page_size=2,
        )
        ...
        assert result.cursor == FeedToken(from_version='0000000000002c03')
        ...
        assert second_params['fromVersion'] == '0000000000002c02'

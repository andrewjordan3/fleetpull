...
class TestRosterStore:
    def test_apply_reflects_zeros_increments_and_evicts(
        ...
        roster_store.apply(
            Provider.MOTIVE, 'vehicles', 'vehicle_id', _zero_delta('a', 'b', 'c')
        )
        roster_store.apply(
            Provider.MOTIVE,
            'vehicles',
        ...
        assert roster_store.read_counts(Provider.MOTIVE, 'vehicles', 'vehicle_id') == {
            'a': 1,
            'b': 1,
            'd': 0,
        }
    ...
    def test_to_zero_resets_an_incremented_count(
        ...
        roster_store.apply(Provider.MOTIVE, 'vehicles', 'vehicle_id', _zero_delta('a'))
        roster_store.apply(
            Provider.MOTIVE,
            'vehicles',
            'vehicle_id',
    ...
    def test_read_members_returns_live_members_ascending(
        ...
        roster_store.apply(
            Provider.MOTIVE, 'vehicles', 'vehicle_id', _zero_delta('c', 'a', 'b')
        )
        assert roster_store.read_members(Provider.MOTIVE, 'vehicles', 'vehicle_id') == [
            'a',
            'b',
            'c',
        ]

    def test_reads_are_scoped_by_feeder_identity(
        self, roster_store: RosterStore
    ) -> None:
        roster_store.apply(Provider.MOTIVE, 'vehicles', 'vehicle_id', _zero_delta('a'))
        roster_store.apply(
            Provider.MOTIVE, 'vehicles', 'other_column', _zero_delta('b')
        )
        roster_store.apply(Provider.SAMSARA, 'vehicles', 'vehicle_id', _zero_delta('c'))
        assert roster_store.read_members(Provider.MOTIVE, 'vehicles', 'vehicle_id') == [
            'a'
        ]
        assert roster_store.read_counts(Provider.MOTIVE, 'vehicles', 'vehicle_id') == {

...
- **SQLite is the single source of truth** for operational state. `metadata.json` files are generated human-readable snapshots the runner projects after each successful run (the run's own counts and window, plus a cursor read-back from the store); the program never reads them.

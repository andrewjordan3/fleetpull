...
| Parity test's FeedMode arm | `tests/api/test_catalog.py:25-41` | Deliberately unmapped: `FeedMode()` → `None`, so the first feed endpoint **forces** the identity-naming decision loudly rather than silently bucketing as `IncrementalEndpoint` |
...
| # | Gap | Size | Notes |
|---|---|---|---|
| G12 | **FeedMode identity naming** — the parity test's deliberately unmapped arm forces it (test_catalog.py:25-41): is a feed identity `IncrementalEndpoint`, or a third type? | **S** (decision + mapping) [G6] |
...
## 2026-07-14 LogRecord feed resolution

GTA-07 is resolved for the `LogRecord` vertical: `FeedMode` is executable through the generic runner, `log_records` uses `GetFeed`, and date-partitioned feed pages commit parquet before the feed token and ledger completion. Single-file feed storage and calculated-feed tombstone interpretation remain deferred.

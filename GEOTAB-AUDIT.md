...
ids and names → `synthetic-…`; ExceptionEvent-style `a`-prefixed GUID-like ids
→ `aSYN` + a zero-padded 19-digit counter (fixed width keeps counter order
lexicographic, preserving captured relative order); no real VINs, serials,
coordinates, company or driver names. Capture the **full** JSON envelope plus **all response headers**
...
**Banked probes (2026-07-15)** — bodies designed and in Andrew's hands;
blocked on the GeoTab account fix; run in this order:

- **P10 — trips window-matching confirmation pair.** The two held bodies
  (widen the window end past the excluded trip's stop; move the window
  start past the earliest captured stop). Confirms stop-matching and gates
  the decided trips fix; nothing backfills before this lands. Off-prediction
  results on either body → stop and re-derive the matching rule.
- **P11 — ExceptionEvent sort trio.** The three held bodies: exact retry of
  the `-32000` composition (deterministic vs transient); `sort` with no
  `search`; `sort` with a dates-only `ExceptionEventSearch`. The third body
  selects the `exception_events` paging arm (seek vs bisection — DESIGN
  §8's 2026-07-15 decision block).
- **P12 — ExceptionEvent window-matching pair.** Idling `ruleSearch` +
  `fromDate 2026-07-06T13:30:00Z` / `toDate 2026-07-06T13:50:00Z`,
  `resultsLimit: 5000`, no sort; read by presence of the two captured
  events straddling the window bounds, never by count: the end-straddler
  only → `activeFrom`-matching; the start-straddler only →
  `activeTo`-matching; both → overlap; neither → containment. Validity
  requires the page under 5,000. The confirming body reverses the roles
  around the third captured event: `fromDate 2026-07-06T19:00:00Z` /
  `toDate 2026-07-06T19:30:00Z`. The two bodies disagreeing → stop and
  re-derive.
- **P13 — ExceptionEvent cap confirmation.** One `GetCountOf`
  `typeName: ExceptionEvent` (no search) and one bare `Get` at
  `resultsLimit: 5000`. A count above 5,000 with the `Get` returning
  exactly 5,000 confirms the silent cap for this type — the precondition
...
new `a`-prefix arm): the three-record idling trio (whole-second and
fractional TimeSpan durations; the `duration = activeTo − activeFrom`
arithmetic, sub-second included; the mutation/late-creation timestamps),
the `-32000 GenericException` envelope with its exact triggering body, the
sort-free succeeding body, and the wrong-referent silent-empty response.
